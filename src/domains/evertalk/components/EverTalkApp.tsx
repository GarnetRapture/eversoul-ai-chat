...
export function EverTalkApp() {
    ...
    if (controller.setupStage !== 'done') {
        return (<SetupWizard open={true} stage={controller.setupStage} language={controller.appLanguage} inferenceMode={controller.inferenceMode} apiProvider={controller.apiProvider} apiKey={controller.apiKey} tier={controller.appSettings?.performance_tier ?? 'balanced'} hardwareProfile={controller.hardwareProfile} downloadProgress={controller.downloadProgress} downloadError={controller.downloadError} isDownloading={controller.isDownloading} labels={controller.labels} onSelectLanguage={controller.setLanguage} onSelectInferenceMode={controller.setSetupInferenceMode} onSelectApiProvider={controller.setSetupApiProvider} onChangeApiKey={controller.setSetupApiKey} onStartDownload={controller.startModelDownload} onNextStage={controller.goToPerformanceStage} onSelectTier={controller.setPerformanceTier} onCompleteSetup={controller.completeSetup}/>);
    }
    return (
    <>
      ...
      <SettingsPanel open={controller.settingsOpen} settings={controller.appSettings} modelValidation={controller.modelValidation} llmSessionStatuses={controller.llmSessionStatuses} llmRequestStatuses={controller.llmRequestStatuses} isResetting={controller.isResetting} resetSummary={controller.resetSummary} resetError={controller.resetError} importedModules={controller.importedModules} moduleImportError={controller.moduleImportError} labels={controller.labels} onClose={controller.closeSettings} onReset={controller.resetAppData} onSetLanguage={controller.setLanguage} onSetShowReasoning={controller.setShowReasoning} onSetInferenceMode={controller.setInferenceMode} onSetApiProvider={controller.setApiProvider} onSetApiKey={controller.setApiKey} onSetExternalApiConfig={controller.setExternalApiConfig} onTestExternalApi={controller.testExternalApi} onImportModule={controller.importModule} onSetModuleEnabled={controller.setModuleEnabled} onDeleteModule={controller.deleteModule}/>
