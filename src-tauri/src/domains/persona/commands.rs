...
pub fn persona_list(
    ...
    let conn = db_state.0.lock().map_err(|e| PersonaError::Database(e.to_string()))?;
...
pub fn persona_update(
    ...
    let conn = db_state.0.lock().map_err(|e| PersonaError::Database(e.to_string()))?;
    let service = PersonaService::new(&conn);
    let req = UpdatePersonaRequest { id, system_prompt, greeting };
...
pub fn persona_get_pack(name_en: String) -> Result<serde_json::Value, PersonaError> {
    PersonaLoader::load_persona(&name_en)
